...
def run_benchmark(num_requests):
    ...
    data = '{"model": "mistral", "prompt": "Hello, world!"}'  # Adjust model and prompt as needed
...
def main():
    ...
    print(f"Average requests with session time: {avg_requests_session_time:.4f} seconds")

    # Determine the fastest method
    if avg_pycurl_time < avg_requests_time and avg_pycurl_time < avg_requests_session_time:
        faster = "pyCurl"
        difference = min(avg_requests_time, avg_requests_session_time) - avg_pycurl_time
    elif avg_requests_time < avg_pycurl_time and avg_requests_time < avg_requests_session_time:
        faster = "requests"
    ...
    percentage = (difference / max(avg_pycurl_time, avg_requests_time, avg_requests_session_time)) * 100
